...
module.exports = function(router) {
  ...
  router.post(
    ...
    async function(req, res) {
      ...
      if (instance_object.get("status") === "cancelled") {
        ...
        res
          .status(400)
          .json({ error: "Instance is not cancelled, cannot be reactivated" });
      }
    },
  );
  ...
  router.post(
    ...
    async function(req, res) {
      const instance_object = res.locals.valid_object;
      try {
        const result = await instance_object.unsubscribe();
        res.json(result);
      } catch (err) {
        console.error(err);
        res.json(err);
  ...
  router.post(`/service-instances`, function(req, res) {
    res.sendStatus(404);
  });
