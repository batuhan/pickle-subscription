...
const getEnabledPlugins = async function(db) {
  const pluginTableExists = await db.schema.hasTable(PLUGIN_TABLE);
...
let basePlugins = function() {
  return [
    ...
    {
      path: `${PLUGIN_DIRECTORY}/updates`,
      interval: 86400000,
      master: "https://hub.serviceshop.io/api/v1/announcements",
    },
...
let getInitialConfig = function() {
  return {
    ...
    company_name:
      process.env.VIRTUAL_HOST && process.env.VIRTUAL_HOST.split(".")[0],
