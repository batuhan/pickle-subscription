    ...
    .then(function(invoice) {
      ...
      return new Promise(async function(resolve, reject) {
        ...
        if (invoice.data.amount_due > 0) {
          ...
          invoice.data.parsed_amount_due = `${currMap(
            store.getState().options.currency || "USD",
          )}${(invoice.data.amount_due / 100)
            .toFixed(2)
            .replace(/\d(?=(\d{3})+\.)/g, "$&,")}`;
          ...
          if (accessManager) {
            ...
            invoice.data.billing_settings_url = accessManager.createLink(
              invoice.data.user_id,
              token,
            );
          }
