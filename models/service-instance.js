...
const buildPayStructure = function (payment_object, callback) {
    const self = this;
    const plan_arr = ['name', 'amount', 'currency', 'interval', 'interval_count', 'statement_descriptor', 'trial_period_days'];
    const random_code = Math.random().toString(36).substring(10, 12) + Math.random().toString(36).substring(10, 12);
    const default_plan = {
...
ServiceInstance.prototype.createPayPlan = async function (plan = null) {
    if (plan === null) {
        const paymentStructure = (await PaymentStructureTemplate.find({id: this.data.payment_structure_template_id}))[0];
...
const requestCancellation = function (callback) {
    const self = this;
    const approve_cancellation = store.getState().options.auto_approve_cancellations
    // Making sure there is only one cancellation request
    const allowed_cancellation_status = ['running', 'requested', 'waiting', 'in_progress'];
    if (allowed_cancellation_status.includes(self.data.status)) {
        ...
        const newServiceCancellation = new ServiceInstanceCancellations(cancellationData);
...
ServiceInstance.prototype.changeProperties = async function (properties) {
    ...
    const oldProperties = updatedInstance.data.references.service_instance_properties;
    ...
    const paymentStructure = (await PaymentStructureTemplate.find({id : updatedInstance.data.payment_structure_template_id}, true))[0];
    ...
    if (this.get("type") === "subscription") {
        ...
        if (paymentPlan === null || paymentPlan.amount === null) {
            ...
            const newPrice = require("../lib/handleInputs").getPrice(newProps, handlers, basePrice);
...
ServiceInstance.prototype.applyPaymentStructure = async function (paymentStructureId, ignoreTrial) {
    ...
    const {payment_plan, references : {service_instance_properties, payment_structure_templates}} = instance_object.data;
    ...
    const paymentStructureTemplate = (await PaymentStructureTemplate.find({id: paymentStructureId}))[0];
    ...
    const newPrice = require("../lib/handleInputs").getPrice(applicableProperties, handlers, paymentStructureTemplate.data.amount);
    ...
    const newInstance = await (await instance_object.changePaymentPlan(paymentPlan, ignoreTrial)).attachReferences();
...
ServiceInstance.prototype.changePaymentPlan = async function (newPlan, ignorePlanTrial, disableProration) {
    ...
    if (this.data.subscription_id !== null) {
        ...
        const stripeSubscription = await Stripe().connection.subscriptions.update(this.data.subscription_id, payload);
...
ServiceInstance.prototype.scheduleCancellation = async function(endDate){
    ...
    if(subscription && subscription.status !== "canceled"){
        if(endDate){
            ...
            const job = schedule.scheduleJob(endDate, this.finishCancellation.bind(this));
            let {lifecycleManager} = store.getState(true).pluginbot.services;
            if (lifecycleManager) {
                lifecycleManager = lifecycleManager[0];
