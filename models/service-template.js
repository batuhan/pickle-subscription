...
const references = [
  ...
  {
    model: User,
    referenceField: "created_by",
    direction: "to",
    readOnly: true,
  },
  {
    model: Tier,
    referenceField: "service_template_id",
    direction: "from",
    readOnly: false,
  },
];
...
ServiceTemplate.prototype.requestPromise = async function(instanceRequest) {
  try {
    ...
    const paymentStructure = (await PaymentStructureTemplate.find({
      id: instanceRequest.payment_structure_template_id,
    }))[0];
    if (self.data.detail) {
      // todo : strip out XSS
    ...
    const plan =
      paymentStructure.data.type === "one_time" ||
      paymentStructure.data.type === "custom" ||
      paymentStructure.data.type === "split"
        ? { ...paymentStructure.data, amount: 0, interval: "day" }
        : { ...paymentStructure.data, ...instanceRequest };
    const payStructure =
      (instanceRequest.amount === 0 &&
        paymentStructure.data.type !== "subscription") ||
      instanceRequest.amount === undefined
        ? null
