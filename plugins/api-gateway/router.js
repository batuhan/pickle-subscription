...
// todo: add authentication logic somewhere not in app.js
            ...
            if (req.isAuthenticated()) {
                ...
                if (req.params.id && permissions.find(needsOwner) && (await Resource.isOwner(req.params.id, req.user.data.id))) {
                    permissions = permissions.reduce((acc, permission) => {
                        const filteredPermission = needsOwner(permission) ? (Array.isArray(permission) && permission.filter(perm => !needsOwner(perm))) : permission;
    ...
    while (true) {
        ...
        const {ResourceDefinition, endpoint, method, middleware, permissions, description} = yield consume(routeDefinition);
        console.log("Received route definition", endpoint);
        const newRoute = require("express").Router();

        const routePath = ResourceDefinition ? `/${ResourceDefinition.name}${endpoint}` : endpoint;
