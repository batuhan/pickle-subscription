module.exports = {
  async up(knex) {
    const ServiceTemplate = require("../../../../models/base/entity")(
      "service_templates",
      [],
      "id",
      knex,
    ...
    await knex.schema.createTable("payment_structure_templates", table => {
      ...
      table
        .enu("type", ["subscription", "one_time", "custom", "split"])
    ...
    const createdPayments = await knex("payment_structure_templates").insert(
      paymentTemplates,
    );
    ...
    await knex.schema.alterTable("service_instances", table => {
      table
        .integer("payment_structure_template_id")
        .references("payment_structure_templates.id");
    });
    return await knex;
  },

  async down(knex) {
    const paymentStructures = await knex(
    ...
    for (const struct of paymentStructures) {
      const { service_template_id } = tiers.find(
        tier => tier.id === struct.tier_id,
      );
      delete struct.id;
      delete struct.tier_id;
      await knex("service_templates")
        .update(struct)
        .where("id", service_template_id);
    }
