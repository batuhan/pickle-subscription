module.exports = function (database, initConfig) {
    ...
    const assignPermissionPromise = function (initConfig, permission_objects, initialRoleMap) {
        return function (role) {
            return new Promise(function (resolve, reject) {
                const mapped = initialRoleMap[role.get("role_name")];
                const perms_to_assign = permission_objects.filter(p => mapped.includes(p.get("permission_name")));
    ...
    return new Promise(function (resolve, reject) {
            ...
            const permissions = [...(new Set([...initialRoleMap.admin, ...initialRoleMap.user, ...initialRoleMap.staff]))];
            const roles = Object.keys(initialRoleMap);
            const permission_data = permissions.map(permission => ({"permission_name": permission}));
            ...
            const defaultCategory = new ServiceCategory({name: 'Uncategorized', description: 'Uncategorized Services'});
            ...
            NotificationTemplate.batchCreate(DefaultTemplates.templates, function (emailResult) {
                ...
                Role.batchCreate(role_data, function (roles) {
                    ...
                    SystemOption.batchCreate(systemOptions, function (optionResult) {
                        ...
                        const EmailTemplateToRoles = require("../../models/base/entity")("notification_templates_to_roles", [], "id", database);
                        ...
                        Permission.batchCreate(permission_data, function (result) {
                            ...
                            const permission_objects = result.map(permission => new Permission(permission));
