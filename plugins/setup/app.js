...
module.exports = function*(appConfig, initialConfig, dbConfig, app) {
  const channel = eventChannel(emitter => {
    ...
    if (
      dbConfig &&
      initialConfig.admin_user &&
      initialConfig.admin_password &&
      initialConfig.company_name &&
      initialConfig.company_email
    ) {
      console.log("Configuration pre-set, initialization starting");
      require("../../bin/setup")({ ...initialConfig, ...dbConfig }, function(
        env,
      ) {
    ...
    api.get(`/api/v1/system-options/file/brand_logo`, function(req, res, next) {
      ...
      return res.sendFile(
        path.resolve(
          __dirname,
          "../../public/assets/logos/servicebot-logo.png",
        ),
      );
    });
            ...
            .then(function(exists) {
              if (exists.length > 0) {
                res
                  .status(200)
                  .json({
                    message: "Connected to an Existing Database",
                    empty: false,
                  });
              } else {
                res
                  .status(200)
                  .json({
                    message: "Connected to Empty Database",
                    empty: true,
                  });
              }
            });
    ...
    api.post("/api/v1/check-stripe", function(req, res, next) {
      ...
      require("../../lib/stripeValidator")(publishable, secret, function(
        err,
        result,
      ) {
        if (err) {
