...
module.exports = function(knex) {
  ...
  const importStripeUsers = function(last_id, notifyUsers, protocol, host) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      stripe().connection.customers.list(req_params, function(err, customers) {
        if (!err) {
          Promise.all(
            customers.data.map(function(customer) {
              return importUser(customer, notifyUsers, protocol, host).catch(
                function(err) {
                  console.log(err);
            ...
            if (customers.has_more) {
              ...
              console.log(
                `Parsed all users until Customer ID ${last_customer}`,
              );
              return setTimeout(
                resolve,
                1000,
        ...
        stripe().connection.plans.list(req_params, async function(err, plans) {
          if (!err) {
            for (const plan of plans.data) {
              await importServiceTemplate(category, plan).catch(err =>
  ...
  const importStripeSubscriptions = function(last_id) {
    ...
    return new Promise(function(resolve, reject) {
      stripe().connection.subscriptions.list(req_params, async function(
        err,
        subscriptions,
      ) {
        if (!err) {
          for (const subscription of subscriptions.data) {
            await importServiceInstance(subscription).catch(err => {
              console.error(err);
            });
          }
          ...
          if (subscriptions.has_more) {
            const last_subscription =
              subscriptions.data[subscriptions.data.length - 1].id;
            console.log(
              `Parsed all subscriptions until Plan ID ${last_subscription}`,
            );
            return setTimeout(
              resolve,
              1000,
            ...
            new_user.create(function(err, user) {
              if (!err) {
                ...
                invite.create((err, result) => {
                  ...
                  if (notifyUsers) {
                    console.log(
                      `going to send email to stripe invited user ${user.get(
                        "email",
                      )}`,
                    );
  ...
  let importServiceTemplate = function(category, plan) {
    return new Promise(function(resolve, reject) {
      ServiceTemplate.findOne(
        "name",
        plan.name || plan.nickname || plan.id,
        function(template) {
          if (template.data) {
      ...
      return new Promise(async function(resolve, reject) {
        if (!template) {
          ...
          newTemplate.description =
            plan.name || plan.nickname || "Imported from Stripe";
    ...
    return new Promise(function(resolve, reject) {
      ServiceCategory.findOne("name", "Uncategorized", function(category) {
        if (!category.data) {
          ...
          category_obj.create((err, category) => {
  ...
  let importServiceInstance = function(subscription) {
    return new Promise(function(resolve, reject) {
      ...
      ServiceInstance.findOne("subscription_id", subscription.id, function(
        service,
      ) {
        if (!service.data) {
          return resolve(true);
      ...
      .then(function(service) {
        return new Promise(async function(resolve, reject) {
          ...
          const template = (await ServiceTemplate.find(
            { name: plan.name || plan.nickname || plan.id },
            true,
          ))[0];
          if (template && template.data) {
            const paymentStructureTemplate = (await PaymentStructureTemplate.find(
              { tier_id: template.data.references.tiers[0].id },
            ))[0];
            service.service_id = template.data.id;
            service.payment_structure_template_id =
              paymentStructureTemplate.data.id;
          } else {
            const {
              template,
              newTier,
              newPaymentStructure,
            } = await importServiceTemplate("Uncategorized", plan);
            service.service_id = template.data.id;
      ...
      .then(function(service) {
        return new Promise(function(resolve, reject) {
          ...
          service_instance.create((err, instance) => {
