...
const StripeValidator = require("../../../lib/stripeValidator");
...
module.exports = function(router, knex, stripe) {
  ...
  const promiseStripeReconfigure = function(stripe_secret, stripe_publishable) {
      ...
      .then(function() {
        ...
        return new Promise(function(resolve, reject) {
          if (!Stripe().stripe_secret_key) {
            console.log(
              "No stripe key exists in current environment, proceed with migration",
            );
            return resolve(true);
          }
          if (
            stripe_secret.slice(3, 7) === Stripe().stripe_secret_key.slice(3, 7)
          ) {
            // Check the Stripe account ID of the old and the new keys
            const NewStripe = require("stripe")(stripe_secret);
            NewStripe.account.retrieve(function(err, new_stripe) {
              if (!err) {
                ...
                console.log(
                  "Cannot retrieve new Stripe account info! Aborting the reconfiguration.",
                );
                return reject(
                  "Cannot retrieve new Stripe account info! Aborting the reconfiguration.",
                );
            ...
            console.log(
              "New Stripe environment is different than the existing one. Initiating migration...",
            );
            return resolve(true);
          }
        });
  ...
  const getStripeKeys = function(req, res) {
    SystemOptions.findOne("option", "stripe_secret_key", function(
      option_secret_key,
    ) {
      SystemOptions.findOne("option", "stripe_publishable_key", function(
        option_publishable_key,
      ) {
  ...
  const preconfigure = function(req, res) {
    ...
    promiseStripeReconfigure(stripe_secret, stripe_publishable)
          ...
          if (do_migration) {
            User.findAll(true, true, function(users) {
              Promise.all(
                ...
                .catch(function(err) {
                  console.log(
                    `Purge process on old Stripe account failed. Continue without full removal... - Error: ${err}`,
                  );
                  return resolve_purge(do_migration);
                });
            });
