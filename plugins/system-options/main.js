...
function* getPluginOptions(publicOnly=false, valueOnly=false){
    ...
    const pluginOptions = yield select((state) => state.pluginbot.services.pluginOption)
    const optionEffecs = {};
    ...
    *run (config, provide, services) {
        ...
        const configurationManager = {
            ...
            *getConfigurations(publicOnly=false){
                ...
                const publicOptions = yield call(Settings.find, publicOnly ? {"public" : true} : {});
            ...
            *updateConfigurations(settingsArray, publicOnly=false) {
                ...
                const publicSettings = yield call(configurationManager.getConfigurations, publicOnly)
                const groupedUpdates = settingsArray.reduce((acc, optionToUpdate) => {
                ...
                for(const pluginOptionToUpdate of groupedUpdates.plugin){
                    const setter = mappedPluginOption[pluginOptionToUpdate.option].setOption;
                    if(setter) {
        ...
        const initialPublic = yield call(configurationManager.getConfigurations, true);
