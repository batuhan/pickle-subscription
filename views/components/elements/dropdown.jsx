...
class Dropdown extends React.Component {
  ...
  getButton(item, index) {
    if (item.type == "divider") {
      ...
      return (
        <li key={`item-${index}`}>
          <a role="button" tabIndex={0} onClick={item.action}>
            {item.label}
          </a>
        </li>
      );
      ...
      return (
        <li key={`item-${index}`}>
          <a role="button" tabIndex={0} href={item.action}>
            {item.label}
          </a>
        </li>
      );
  ...
  render() {
    ...
    return (
      <div className={`action-dropdown ${opened ? "open" : "closed"}`}>
        <button
          className="buttons _default _navy _dropdown-toggle"
          type="button"
        ...
        >
          {dropdown.map((item, index) => this.getButton(item, index))}
        </ul>
