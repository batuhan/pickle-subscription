...
class NotificationTemplateForm extends React.Component {
  ...
  humanString(text, capitalize = false) {
    ...
    return capitalize ? (
      <span className={`capitalize`}>{humanText}</span>
    ) : (
      humanText
    );
  }
                    ...
                    <Rows>
                      <Section>
                        ...
                        <div className={`sb-form-group _group-email-subject`}>
                          <input
                            className={`default-input _input- _input-email-subject`}
                            type="text"
                            name="subject"
                        ...
                        <React.Fragment>
                          <h4 className="text-capitalize">
                            {this.humanString(
                              `Available fields for ${template.data.name}`,
                              true,
                            )}
                          </h4>
                          <span className="form-help-text">
                            ...
                            {this.humanString(template.data.name)}) into the
                            body of your notification.
                          </span>
                          <ul className="templateList">
                            {Object.keys(template.schema).map(field => {
                              if (field === "references") {
                                return Object.keys(references).map(
                                  reference => {
                                    return (
                                        ...
                                        >
                                          {Object.keys(
                                            ...
                                            return (
                                              <li
                                                key={referenceColumn}
                                                className="column reference-column"
                                              >
                                                <button
                                                  className="buttons btn-sm btn-info"
                        ...
                        <div className="p-t-15">
                          <Buttons
                            buttonClass={`__save-email-template`}
                            btnType="primary"
