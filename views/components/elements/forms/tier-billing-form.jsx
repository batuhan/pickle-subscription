...
let PaymentStructureTemplates = function(props) {
  ...
  let optionMap = plans.map(plan => {
    return availableOptions.filter(
      option =>
        option.id === plan.interval ||
        !plans.some(plan2 => plan2.interval === option.id),
    );
  });
  // ].filter(option => {
  ...
  if (tier.type === "custom") {
    return (
      <div>
        <p>
          Quotes are built for services that are customer specific. If your
          service is priced based on the customer's use-case, use this option.
          Once the quote service has been requested by the customer, you can add
          charges to the service at anytime.
        </p>
      </div>
    );
  }
  ...
  if (tier.type === "subscription") {
    return (
      <div className="payment-structures">
            ...
            <div key={"field-" + index + "-" + member}>
              {index > 0 && (
                <div className="_billing-options">
                  ...
                  <span className="_sub-heading">
                    Billing Option {index + 1}
                  </span>
                  <div className="_hr" />
                  {fields.length > 1 && (
                    <button
                      className="_billing-options-delete"
        ...
        {plans.length < 4 && (
          <button className="buttons _add" onClick={addPayment}>
    ...
    <div className="_tier-details">
      <div className="_tier-name_name">
        <Field
          ...
          label="Tier Name (eg. Basic, Enterprise)"
          validate={[required()]}
        />
      </div>
      ...
      {tier.type === "subscription" && (
        <div className="_tier-name_billing_period">
          <Field
            ...
            format={formatFromPricing}
            name={member + ".trial_period_days"}
            type="number"
            component={inputField}
      ...
      <FormSection name={member + ".references"}>
        <FieldArray
          name="payment_structure_templates"
