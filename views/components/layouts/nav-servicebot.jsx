...
class NavServiceBot extends React.Component {
    ...
    return (
      ...
      this.props.services.routeDefinition.reduce((acc, route, index) => {
        if (
          ...
          (route.navType === "main" || route.navType === undefined)
        ) {
          acc.push(
            <li key={`item-${index}`}>
              <Link
              ...
              >
                {icon && <span className={`nav-icons icon-${icon}`} />}
                {route.name}
              </Link>
            </li>,
          );
      ...
      this.props.services.routeDefinition.reduce((acc, route, index) => {
        if (route.isVisible(user) && route.navType === "settings") {
          acc.push(
            <li key={`item-${index}`}>
              <Link
                className={self.getLinkClass(route.path.substr(1), "child")}
                key={index}
                to={route.path}
              >
                {route.name}
  ...
  getLinkClass(expectedPath, linkType) {
    ...
    if (_.isArray(expectedPath)) {
      return _.includes(expectedPath, path.split("/")[1])
        ? `nav-link-${linkType} active`
        : `nav-link-${linkType}`;
    }
    return path.split("/")[1] === expectedPath
      ? `nav-link-${linkType} active`
      : `nav-link-${linkType}`;
  }
  ...
  getSetupSteps() {
    ...
    let hasStripeKeys =
      ...
      options.stripe_publishable_key.value !== "";
    ...
    if (!setupComplete) {
      return (
        <div className="nav-setup-checklist">
          <Link
            to="/dashboard"
            className={`${getSetupClass("dashboard", "parent")} active`}
          >
            Your Checklist
          </Link>
          <div className="_list">
            <Link
              to="/manage-catalog/create"
              className={getSetupClass("child", hasOffering)}
            >
              <span className="form-step-count _step-count">1</span> Create SaaS
              Offering
            </Link>
            <Link
              to="/stripe-settings"
              className={getSetupClass("child", hasStripeKeys)}
            >
              <span className="form-step-count _step-count">2</span> Add Stripe
              keys
            </Link>
          </div>
        </div>
  ...
  getMenuItems(style) {
    ...
    let defaultSettingLinks = [
      "stripe-settings",
      "notification-templates",
      "system-settings",
    ];
    let customSettingRoutes =
      this.props.services.routeDefinition &&
      this.props.services.routeDefinition.reduce((acc, route, index) => {
    ...
    if (isAuthorized({ permissions: ["can_administrate", "can_manage"] })) {
        ...
        <ul className="app-links">
          ...
          <li>
            <Link
              to="/manage-catalog"
              style={style}
              className={getLinkClass("manage-catalog", "parent")}
            >
              <span className="nav-icons icon-manage" />
          ...
          <li>
            <Link
              to="/manage-users"
              style={style}
              className={getLinkClass("manage-users", "parent")}
            >
              <span className="nav-icons icon-users" />
          ...
          <li>
            <Link
              to="/embeddables"
              style={style}
              className={getLinkClass("embeddables", "parent")}
            >
          ...
          <li className="app-dropdown">
            <Link
              className={getLinkClass(linkGroupSettings, "parent")}
              to="/stripe-settings"
            >
              <span className="nav-icons icon-settings" />
              Settings
            ...
            <ul className="app-dropdown">
              <li>
                <Link
                  to="/stripe-settings"
                  className={getLinkClass("stripe-settings", "child")}
                >
                  Stripe Settings
                </Link>
              </li>
              <li>
                <Link
                  ...
                  className={getLinkClass("notification-templates", "child")}
                >
                  Email Settings
                </Link>
              </li>
              <li>
                <Link
                  to="/system-settings"
                  className={getLinkClass("system-settings", "child")}
                >
                  System Settings
                </Link>
              </li>
              ...
              <li>
                <Link
                  href="https://api-docs.servicebot.io/"
                  target="_blank"
      ...
      return (
        <ul className="app-links">
          ...
          <li>
            <Link to={`/billing-history/${this.props.uid}`}>
              Billing History
            </Link>
          </li>
          <li>
            <Link to={`/billing-settings/${this.props.uid}`}>
              Payment Method
            </Link>
          </li>
          {this.getPluginItems()}
  ...
  getLivemode() {
    ...
    if (pk === undefined) {
      return (
        <span
          data-tip
          data-for="notification-stripe-keys"
    ...
    if (livemode.toUpperCase() === "TEST") {
      return (
        <span className="notification-badge stripe test m-0">
          <strong>Stripe Test Mode</strong>
        </span>
      );
    } else {
      return (
        <span className="notification-badge stripe live m-0">
        ...
        <div className="app-layout">
          <div className="app-header">
            <div className="app-header-left">
              <Link to="/">
                <img
                  className="app-logo"
                  src="/api/v1/system-options/file/brand_logo"
                />
              </Link>
            </div>
            <Authorizer>
              <div className="app-header-right">
                <div className="app-profile">
                  <Link to="/profile">
                    <img
                      ...
                      src={`/api/v1/users/${this.props.uid}/avatar`}
                      ref="avatar"
                      alt="profile image"
                    />
                ...
                <NavNotification />
                <button
                  className="buttons _text logout"
          ...
          <div className="app-navigation">
            <nav className="app-links-container">
              <div className="_main">
                <Authorizer>{this.getMenuItems(linkTextStyle)}</Authorizer>
                <Authorizer anonymous={true}>
                  <VisibleAnonymousLinks />
                </Authorizer>

                <div className="nav-footer">
                  <div className="navvbar-badge">{this.getLivemode()}</div>
...
export default consume("routeDefinition", "footerComponent")(
  connect(mapStateToProps)(NavServiceBot),
);
