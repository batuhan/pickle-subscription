...
class BillingHistoryList extends React.Component {
  ...
  fetchUser() {
    let self = this;
    Fetcher(`/api/v1/users/${self.props.uid}`)
  ...
  modAmountDue(data, resObj) {
    return <Price value={data} currency={resObj.currency} />;
  }
  modDate(data) {
      ...
      <Dropdown
        ...
        dropdown={[
          ...
          },
          {
            ...
            action: () => {
              browserHistory.push(
                `/service-instance/${row.service_instance_id}`,
              );
            },
          },
          {
          ...
          },
          {
            ...
            label: "Make Refund",
            action: () => {
              self.openRefundModal(row);
            },
          },
        ].filter(option => {
          if (
            option.label !== "Make Refund" ||
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <ServiceBotTableBase rows={rows} sortColumn="date" sortOrder="desc">
          <TableHeaderColumn
            isKey
            dataField="id"
            dataSort={true}
