...
let EmbeddableCard = function(props) {
  ...
  return (
    <div
      className={`embeddable-card ${selected && "selected"}`}
...
class Embeddables extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.params &&
      this.props.params.embedName &&
      (!prevProps.params ||
        !prevProps.params.embedName ||
        this.props.params.embedName !== prevProps.params.embedName)
    ) {
  ...
  render() {
    ...
    let selected = embeddables.find(
      embed => slug(embed.name) === self.state.selected,
    );
            ...
            {embeddables.map((embeddable, index) => {
              if (self.state.selected === slug(embeddable.name)) {
                ...
                return (
                  <EmbeddableCard
                    key={`item-${index}`}
                    onClick={select(slug(embeddable.name))}
                    {...embeddable}
                  />
                );
              }
  ...
  return (
    <div className="embeddable-intro">
      <div className="_content-image">
        <img
          src="/assets/embed/embed-intro-default.png"
          alt="Embed Intro Image"
        />
      </div>
      <p className="_content-text">
        To get started, choose an embedabble that you would like to add to you
        website. Choose from the widgets list on the left hand side.
      </p>
    </div>
  );
};
