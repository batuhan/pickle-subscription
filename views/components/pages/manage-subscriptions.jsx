...
class ManageSubscriptions extends React.Component {
  ...
  async fetchData() {
    ...
    if (this.props.params.status) {
      if (status === "running") {
        ...
        url =
          "/api/v1/service-instances/search?key=status&value=waiting_cancellation";
      }
    }
    if (uid) {
  ...
  typeFormatter(cell, row) {
    if (cell) {
      ...
      const formattedType = {
        subscription: (
          <Badge type={`default`}>
            <i className="fa fa-circle micro-badge black" /> {label[cell]}{" "}
          </Badge>
        ),
        custom: (
          <Badge type={`default`}>
            <i className="fa fa-circle micro-badge grey" />{" "}
            {getBillingType(row)}
          </Badge>
        ),
        one_time: (
  ...
  statusDataValue(cell, row) {
    ...
    if (status === "running") {
      ...
      if (currentDate < trialEnd) status = "trialing";
      else status = "active";
  ...
  amountFormatter(cell, row) {
    if (cell >= 0) {
      return (
        <Badge type={`default`}>
          <Price value={cell} currency={row.currency} />
        </Badge>
      );
    } else {
  ...
  statusFormatter(cell, row) {
    switch (cell) {
      ...
      case "waiting_cancellation":
        return (
          <span className="status-badge yellow">Waiting Cancellation</span>
        );
  ...
  createdFormatter(cell, row) {
    let fullTime = `${getFormattedDate(cell, {
      time: true,
    })} - Created: ${getFormattedDate(row.created_at, { time: true })}`;
    return (
      <div className="datatable-date">
        <span data-tip={fullTime} data-for="date-updated">
        ...
        dropdown={[
          {
            ...
            label: "View",
            action: () => {
              browserHistory.push(`/service-instance/${row.id}`);
            },
          },
          { type: "divider" },
          {
            ...
            label: "View Invoice",
            action: () => {
              browserHistory.push(`/billing-history/${row.user_id}`);
            },
          },
          customAction,
        ]}
  ...
  render() {
    ...
    let {
      ...
      allUsers,
      currentDataObject,
      currentDataObject: { status, payment_plan },
      actionModal,
    } = this.state;
    ...
    const getPaymentData = (row, users) => {
      ...
      if (user.references.funds.length) {
        if (user.status === "flagged") return "failed";
        else return "paying";
      }
      return "no payment";
    };
            ...
            <Content>
              ...
              >
                <TableHeaderColumn
                  ...
                  dataFormat={(cell, row) => {
                    return (
                      <Link to={`/service-instance/${row.id}`}>{cell}</Link>
                    );
                  }}
                  dataSort={true}
                  searchable={true}
