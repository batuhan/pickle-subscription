  ...
  openEditRole(dataObject) {
    ...
    if (dataObject.id == self.props.user.id) {
      self.setState({
        openMessageModal: {
          title: "Alert!",
          message: "Editing own role is not allowed!",
        },
      });
    } else {
  ...
  openDeleteUser(dataObject) {
    if (dataObject.id == this.props.user.id) {
      this.setState({
        openMessageModal: {
          title: "Alert!",
          message: "Deleting own user is not allowed!",
        },
      });
    } else {
      this.setState({
    ...
    let dropdownOptions = [
      {
        ...
        action: () => {
          return this.openInviteUserModal(row);
        },
      },
      {
        type: "button",
        label: row.references.funds.length
          ? "Edit Credit Card"
          : "Add Credit Card",
        action: () => {
          return this.openEditCreditCard(row);
        },
      },
      {
        type: "button",
        label: "Edit User",
        action: () => {
          browserHistory.push(`/manage-users/${row.id}`);
        },
      },
      {
        type: "button",
        ...
        action: () => {
          browserHistory.push(`/manage-subscriptions/?uid=${row.id}`);
        },
      },
      { type: "divider" },
      {
      ...
      },
      {
        ...
        action: () => {
          return this.openSuspendUser(row);
        },
      },
      {
        type: "button",
        label: "Delete User",
        action: () => {
          return this.openDeleteUser(row);
        },
      },
    ].filter((option, index) => {
      ...
      if (
        self.props.user &&
        self.props.user.id === row.id &&
        (option.label === "Delete User" ||
          option.label === "Edit Role" ||
          option.label === "Suspend User")
      ) {
        return false;
      }
...
export default connect(state => {
  return {
    ...
    stripe_publishable_key:
      state.options && state.options.stripe_publishable_key,
  };
})(ManageUsers);
