      ...
      return (
        <div>
          <div
            className="_backdrop"
            onClick={() => {
              self.setState({ openNotificationDropdown: false });
            }}
          />
          <div className="app-notification-list">
            ...
            {totalUnread ? (
              <h4 className="__unread-heading">{`You have ${totalUnread} unread notifications`}</h4>
            ) : (
              <React.Fragment />
            )}
            {!totalUnread ? (
              <h4 className="__empty-heading">You have no new notifications</h4>
            ) : (
              <React.Fragment />
            )}
            <ul className={`__unread-list`}>
              ...
              <li className={`_view-all`}>
                <button className={`buttons _text`} onClick={this.viewAll}>
                  View All
                </button>
              </li>
            </ul>
    ...
    return (
      <div className="app-notification">
        <span className="_toggle" onClick={this.openNotificationDropdown}>
          ...
          {unread.length ? (
            <span className="nav-notification-indicator" />
          ) : (
            <span />
          )}
        ...
        {this.state.openNotificationDropdown && (
          <div
        ...
        {this.state.viewMessage !== null && (
          <ModalNotification
            ...
            hide={this.closeMessageModel}
            notification={this.state.viewMessage}
          />
        )}
      </div>
    );
...
class NotificationList extends React.Component {
  ...
  modMessage(data, dataObj) {
    if (data.length <= 90) {
      return (
        <span
          className="buttons _default _text __message-link"
          onClick={() => {
      ...
      return (
        <span
          className="buttons _default _text __message-link-trimmed"
          onClick={() => {
            return this.openMessageModel(dataObj);
          }}
  ...
  render() {
    ...
    if (notifications.length) {
      ...
      return <p>{notificationsNullMessage}</p>;
    }
  }
}
...
function mapDispatchToProps(dispatch) {
  return {
    setNotifications: (notifications, system = false) => {
      return dispatch(setNotifications(notifications, system));
    },
    setNotification: (notification, system = false) => {
      return dispatch(setNotification(notification, system));
    },
    setSystemNotifications: (notifications, system = true) => {
      return dispatch(setSystemNotifications(notifications, system));
    },

    addNotification: (notification, system = false) => {
      return dispatch(addNotification(notification, system));
    },
  };
}
