...
class ImageUploader extends React.Component {
  ...
  handleImage(e) {
    ...
    const init = {
      ...
      credentials: "include",
      body: new FormData(
        document.getElementById(`imgform${this.state.elementID}`),
      ),
    };
    self.setState({ ajaxLoad: true });
    Fetcher(self.state.imageURL, null, null, init)
        ...
        if (!result.error) {
          self.setState({ imageSelected: false, ajaxLoad: false }, function() {
            ...
            if (self.props.reloadNotice) {
              self.setState({
                success: true,
                reloadNotice: self.props.reloadNotice,
              });
            }
          });
        } else {
      ...
      .catch(e => {
        console.error("error getting img", e);
      });
  }
  ...
  removeImage(e) {
    ...
    Fetcher(self.props.imageURL, "DELETE", null, null)
      ...
      .catch(e => {
        console.error("error removing img", e);
      });
  }
    ...
    return (
      <div className="row">
        <div className={`col-md-12 edit-${this.state.elementID}-image`}>
          <form
            id={`imgform${this.state.elementID}`}
            className="image-uploader"
            encType="multipart/form-data"
          >
            <div className={`${this.state.imageStyle}`}>
              <img
                id={`edit-${this.state.elementID}-img`}
                className={!this.state.image && "hidden"}
                src={this.props.imageGETURL || this.state.imageURL}
                ref="avatar"
                alt="badge"
              />
            ...
            {this.state.success && this.state.reloadNotice && (
              <div>
                <span className="help-block">
                  <small>{this.props.reloadNotice}</small>
                </span>
                <Buttons
                  btnType="primary"
            ...
            {this.state.imageSelected && this.state.uploadButton && (
              <div>
                <div className="image-upload-message">
                  <small>{this.state.uploadMessage}</small>
                </div>
                <Buttons
                  btnType="primary"
                  text="Save Image"
          ...
          {this.state.imageFailed && (
            <span className="help-block image-failed">
              {this.state.imageFailed}
            </span>
          )}
        </div>
