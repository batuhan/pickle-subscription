...
class Inputs extends React.Component {
      ...
      return React.Children.map(self.props.children, child => {
        let newProps = dependencies.map(dep => {
          if (child.props.name == dep.dependsOn) {
            let dependentChild = React.Children.map(
              self.props.children,
              child => {
  ...
  render() {
    ...
    if (
      ...
      return (
        <div
          className={`sb-form-group${warning ? "has-warning" : ""} ${
            error ? "has-error" : ""
          } ${type == "hidden" ? "hidden" : ""}`}
        >
      ...
      return (
        <div
          ...
          className={`sb-form-group color-picker-input ${
            error ? "has-error" : ""
          }`}
        >
          {label && <label className="_label- text-capitalize">{label}</label>}
