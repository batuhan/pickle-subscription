...
let RenderWidget = props => {
  ...
  const widget =
    ...
    props.services.widget.find(
      widgetToCheck => widgetToCheck.type === widgetType,
    );
  if (!widget) {
          ...
          <div className="addon-widget-has-pricing">
            <FormSection name="pricing">
              ...
              <div className="form-group form-group-flex addon-widget-pricing-inputs-wrapper">
                <label className="control-label form-label-flex-md addon-widget-pricing-input-label">
                  Add-On Pricing
                </label>
                <Field
                  name="value"
...
let PriceBreakdown = props => {
  ...
  let breakdown = inputs.reduce((acc, input) => {
    if (
      ...
      widgets[input.type].handler.priceHandler
    ) {
      acc.push(
        <div>
          {input.prop_label}
