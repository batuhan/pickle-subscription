...
function optionsReducer(
  state = cookie.load("spk")
    ? {
        stripe_publishable_key: {
          option: "stripe_publishable_key",
          data_type: "hidden",
          value: cookie.load("spk"),
        },
      }
    : {},
  action,
...
function permissionReducer(
  state = (localStorage.getItem("permissions") &&
...
const initializedState = function(initialOptions = null) {
  return async function(dispatch) {
    ...
    initialState.options =
      initialOptions || (await Fetcher("/api/v1/system-options/public"));
    try {
      if (cookie.load("uid")) {
        ...
        if (initialState.user.status === "invited") {
          initialState.alerts = [
            ...initialState.alerts,
            {
              id: "1",
              message:
                "Please check your email and set your password to complete your account.",
              show: true,
            },
          ];
        ...
        if (isAuthorized({ permissions: "put_email_templates_id" })) {
          initialState.system_notifications = await Fetcher(
